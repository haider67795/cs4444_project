...
void spmspm_cpu(CSRMatrix* csrMatrix1, CSRMatrix* csrMatrix2,
                COOMatrix* cooMatrix) {
  ...
  unsigned int* outputCols =
      (unsigned int*)malloc(csrMatrix2->numCols * sizeof(unsigned int));
  unsigned int numOutputCols = 0;
  for (unsigned int row1 = 0; row1 < csrMatrix1->numRows; ++row1) {
    for (unsigned int i1 = csrMatrix1->rowPtrs[row1];
         i1 < csrMatrix1->rowPtrs[row1 + 1]; ++i1) {
      unsigned int col1 = csrMatrix1->colIdxs[i1];
      float value1 = csrMatrix1->values[i1];
      unsigned int row2 = col1;
      for (unsigned int i2 = csrMatrix2->rowPtrs[row2];
           i2 < csrMatrix2->rowPtrs[row2 + 1]; ++i2) {
        ...
        if (oldVal == 0.0f) {  // Assuming all matrix entries are positive, so
                               // oldVal cannot become 0 again
...
void verify(COOMatrix* cooMatrixGPU, COOMatrix* cooMatrixCPU,
            unsigned int quickVerify) {
  if (cooMatrixCPU->numNonzeros != cooMatrixGPU->numNonzeros) {
    printf(
        "    \033[1;31mMismatching number of non-zeros (CPU result = %d, GPU "
        "result = %d)\033[0m\n",
    ...
    printf(
        "        This verification is not exact. For exact verification, pass "
        "the -v flag.\n");
  } else {
    ...
    for (unsigned int i = 0; i < cooMatrixCPU->numNonzeros; ++i) {
      ...
      if (rowCPU != rowGPU || colCPU != colGPU ||
          abs(valGPU - valCPU) / valCPU > 1e-5) {
        printf(
            "        \033[1;31mMismatch detected: CPU: (%d, %d, %f), GPU: (%d, "
            "%d, %f)\033[0m\n",
...
int main(int argc, char** argv) {
  ...
  COOMatrix* cooMatrix = createEmptyCOOMatrix(
      csrMatrix->numRows, csrMatrix->numRows, csrMatrix->numRows * 10000);
  COOMatrix* cooMatrix_h = createEmptyCOOMatrix(
      csrMatrix->numRows, csrMatrix->numRows, csrMatrix->numRows * 10000);
  ...
  if (runGPUVersion0 || runGPUVersion1 || runGPUVersion2 || runGPUVersion3 ||
    ...
    // Allocate GPU memory
    startTime(&timer);
    CSRMatrix* csrMatrix_d = createEmptyCSRMatrixOnGPU(
        csrMatrix->numRows, csrMatrix->numCols, csrMatrix->numNonzeros);
    COOMatrix* cooMatrix_d = createEmptyCOOMatrixOnGPU(
        cooMatrix->numRows, cooMatrix->numCols, cooMatrix->capacity);
    cudaDeviceSynchronize();
    stopTime(&timer);
