...
COOMatrix* createEmptyCOOMatrix(unsigned int numRows, unsigned int numCols,
  ...
  cooMatrix->rowIdxs =
      (unsigned int*)calloc(1, capacity * sizeof(unsigned int));
...
void merge(unsigned int* key1, unsigned int* key2, float* data, int left,
           int mid, int right, unsigned int* key1Aux, unsigned int* key2Aux,
           float* dataAux) {
  int size1 = mid - left + 1;
  int size2 = right - mid;

  unsigned int* key1Left = key1Aux;
  key1Aux += size1;
  unsigned int* key1Right = key1Aux;
  key1Aux += size2;
  unsigned int* key2Left = key2Aux;
  key2Aux += size1;
  unsigned int* key2Right = key2Aux;
  key2Aux += size2;
  float* dataLeft = dataAux;
  dataAux += size1;
  float* dataRight = dataAux;
  dataAux += size2;
  ...
  while (i < size1 && j < size2) {
    if (key1Left[i] < key1Right[j] ||
        key1Left[i] == key1Right[j] && key2Left[i] <= key2Right[j]) {
      key1[k] = key1Left[i];
      key2[k] = key2Left[i];
      data[k] = dataLeft[i];
      ++i;
...
void mergeSort(unsigned int* key1, unsigned int* key2, float* data, int start,
               int end, unsigned int* key1Aux, unsigned int* key2Aux,
               float* dataAux) {
  if (start < end) {
...
void sortCOOMatrix(COOMatrix* cooMatrix) {
  unsigned int* key1Aux =
      (unsigned int*)malloc(cooMatrix->numNonzeros * sizeof(unsigned int));
  unsigned int* key2Aux =
      (unsigned int*)malloc(cooMatrix->numNonzeros * sizeof(unsigned int));
  float* dataAux = (float*)malloc(cooMatrix->numNonzeros * sizeof(float));
  mergeSort(cooMatrix->rowIdxs, cooMatrix->colIdxs, cooMatrix->values, 0,
            cooMatrix->numNonzeros - 1, key1Aux, key2Aux, dataAux);
  free(key1Aux);
  free(key2Aux);
  free(dataAux);
}
...
COOMatrix* createEmptyCOOMatrixOnGPU(unsigned int numRows, unsigned int numCols,
  ...
  cudaMemcpy(cooMatrix, &cooMatrixShadow, sizeof(COOMatrix),
...
void clearCOOMatrixOnGPU(COOMatrix* cooMatrix) {
  COOMatrix cooMatrixShadow;
  cudaMemcpy(&cooMatrixShadow, cooMatrix, sizeof(COOMatrix),
             cudaMemcpyDeviceToHost);
  cudaMemset(cooMatrixShadow.rowIdxs, 0,
             cooMatrixShadow.numNonzeros * sizeof(unsigned int));
  cudaMemset(cooMatrixShadow.colIdxs, 0,
             cooMatrixShadow.numNonzeros * sizeof(unsigned int));
  cudaMemset(cooMatrixShadow.values, 0,
             cooMatrixShadow.numNonzeros * sizeof(unsigned int));
...
void copyCOOMatrixFromGPU(COOMatrix* cooMatrix_d, COOMatrix* cooMatrix_h) {
  COOMatrix cooMatrixShadow;
  cudaMemcpy(&cooMatrixShadow, cooMatrix_d, sizeof(COOMatrix),
             cudaMemcpyDeviceToHost);
  ...
  cudaMemcpy(cooMatrix_h->rowIdxs, cooMatrixShadow.rowIdxs,
             cooMatrixShadow.numNonzeros * sizeof(unsigned int),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(cooMatrix_h->colIdxs, cooMatrixShadow.colIdxs,
             cooMatrixShadow.numNonzeros * sizeof(unsigned int),
             cudaMemcpyDeviceToHost);
  cudaMemcpy(cooMatrix_h->values, cooMatrixShadow.values,
             cooMatrixShadow.numNonzeros * sizeof(float),
             cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
}
...
CSRMatrix* coo2csr(COOMatrix* cooMatrix) {
  ...
  csrMatrix->rowPtrs =
      (unsigned int*)malloc((csrMatrix->numRows + 1) * sizeof(unsigned int));
  csrMatrix->colIdxs =
      (unsigned int*)malloc(csrMatrix->numNonzeros * sizeof(unsigned int));
  csrMatrix->values = (float*)malloc(csrMatrix->numNonzeros * sizeof(float));

  // Histogram rows
  memset(csrMatrix->rowPtrs, 0,
         (csrMatrix->numRows + 1) * sizeof(unsigned int));
  for (unsigned int i = 0; i < cooMatrix->numNonzeros; ++i) {
...
CSRMatrix* createEmptyCSRMatrixOnGPU(unsigned int numRows, unsigned int numCols,
  ...
  cudaMalloc((void**)&csrMatrixShadow.rowPtrs,
             (numRows + 1) * sizeof(unsigned int));
  cudaMalloc((void**)&csrMatrixShadow.colIdxs,
             numNonzeros * sizeof(unsigned int));
  cudaMalloc((void**)&csrMatrixShadow.values, numNonzeros * sizeof(float));

  CSRMatrix* csrMatrix;
  cudaMalloc((void**)&csrMatrix, sizeof(CSRMatrix));
  cudaMemcpy(csrMatrix, &csrMatrixShadow, sizeof(CSRMatrix),
             cudaMemcpyHostToDevice);
  cudaDeviceSynchronize();
...
void freeCSRMatrixOnGPU(CSRMatrix* csrMatrix) {
  CSRMatrix csrMatrixShadow;
  cudaMemcpy(&csrMatrixShadow, csrMatrix, sizeof(CSRMatrix),
             cudaMemcpyDeviceToHost);
...
void copyCSRMatrixToGPU(CSRMatrix* csrMatrix_h, CSRMatrix* csrMatrix_d) {
  CSRMatrix csrMatrixShadow;
  cudaMemcpy(&csrMatrixShadow, csrMatrix_d, sizeof(CSRMatrix),
             cudaMemcpyDeviceToHost);
  ...
  cudaMemcpy(csrMatrixShadow.rowPtrs, csrMatrix_h->rowPtrs,
             (csrMatrix_h->numRows + 1) * sizeof(unsigned int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(csrMatrixShadow.colIdxs, csrMatrix_h->colIdxs,
             csrMatrix_h->numNonzeros * sizeof(unsigned int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(csrMatrixShadow.values, csrMatrix_h->values,
             csrMatrix_h->numNonzeros * sizeof(float), cudaMemcpyHostToDevice);
  cudaDeviceSynchronize();
}
